...
def prepare_MIMIC_dataset(DataPath, OutputFile, NsampPerSubMax:int=None, NsampMax:int=None, win_len:int=7, win_overlap:float=0.5, savePPGData=False):
    ...
    b,a = butter(4, Wn=[0.06, 0.33], btype='bandpass')
    ...
    for idx, dirs in enumerate(SubjectDirs): #process subject by subject under DataPath
        print(f'{datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S")}: Processing subject {idx+1} of {NumSubjects} ({dirs.name}): ', end='', flush=True)
        ...
        for file in DataFiles:
            ...
            for i in range(0, N_win):
                ...
                if savePPGData:
                    plt.clf()
                    plt.ion()
                    plt.plot(ppg_win, label="original")


                    
                    ppg_win_filter = filtfilt(b,a, ppg_win)

                    plt.plot(ppg_win_filter, label='filtered')
                    plt.draw()
                    
                    freqs, spectrum = to_frequency_domain(ppg_win_filter)
    
                    plt.figure(figsize=(12, 6))
                    plt.bar(freqs, np.abs(spectrum), width=(freqs[1]-freqs[0]), align='center', color='b', edgecolor='black')
                    plt.title("Frequency Domain (Bar Chart)")
        ...
        #             OUTPUT = OUTPUT[idx_select,:]
            ...
            #     BP_dataset[-OUTPUT.shape[0]:,:] = OUTPUT  #add OUTPUT to tail of BP_dataset
...
if __name__ == "__main__":
    ...
    if len(argv) > 1:
        ...
        prepare_MIMIC_dataset('.\\Watch', '.\\data\\MIMICIII_WATCH.h5', NsampPerSubMax=3000, NsampMax=None,
