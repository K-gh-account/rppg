...
def read_tfrecord(example, win_len=175):
    tfrecord_format = (
        {
...
def create_dataset(tfrecords_dir, tfrecord_basename, win_len=175, batch_size=32, modus='train'):

    pattern = join(tfrecords_dir,modus,tfrecord_basename + "_" + modus + ".tfrecord")
...
def ppg_retrain_rppg(architecture,
                        DataDir,
                        ResultsDir,
                        CheckpointDir,
                        tensorboard_tag,
    ...
    file_path="2024-04-12_resnet_rppg_retrain_cb.h5"
...
if __name__ == "__main__":
    ...
    if len(argv) > 1:
        ...
        parser.add_argument('--winlen', type=int, default=175,
                            help="length of the ppg windows in samples (default: 175)")
        ...
        architecture = 'resnet'
        experiment_name = datetime.now().strftime("%Y-%d-%m") + '_' + architecture + '_' + 'rppg_retrain'
        HomePath = expanduser("")
        DataDir = join(HomePath,'retrain')
        ResultsDir = join(HomePath,'retrain','results')
        CheckpointDir = join(HomePath,'retrain','checkpoints')
        tb_tag = architecture + '_' + 'rppg_retrain'
        batch_size = 16
        win_len = 175
        lr = None
        N_epochs = 1
