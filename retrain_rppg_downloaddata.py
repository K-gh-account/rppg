...
def retrain_rppg_personalization(data_file,
    ...
    model = get_model('alexnet', (175,1), UseDerivative=True)
    print(model.summary())
    model.compile(
    ...
    model.load_weights("2024-08-09_alexnet_mimic_iii_ppg_nonmixed_pretrain_cb.h5")
    ...
    for i in subjects_list:
        ...
        idx_test = np.where(np.isin(subjects_iter, i))[-1] #返回对应下标，注意，下标是两个维度的，第一个维度都是0
        # subjects_iter = np.delete(subjects_iter, np.where(idx_test))
        subjects_list_iter = np.delete(subjects_list_iter, np.where(np.isin(subjects_list_iter,i))) #列表中删除i
        ...
        idx_val = np.where(np.isin(subjects_iter, subjects_val))[-1] #idx_test为subject i的数据index, idx_train和val为其余数据的index
        ...
        count = 0
        for i in range(rppg_train.shape[0]):
            if count >= 3:
                break
            count += 1
            plt.clf()
            plt.ion()
            plt.plot(rppg_train[i])
            plt.draw()
            plt.pause(0.001)
        ...
        # results = results._append(pd.DataFrame(np.concatenate((
...
if __name__ == "__main__":
    ...
    DataFile = 'data\\rPPG-BP-UKL_rppg_7s.h5'
    ModelFile = '2024-28-07_slapnicar_mimic_iii_ppg_nonmixed_pretrain_cb.h5'
    experiment_name = '2024_08_14_alexnet_rppg'
