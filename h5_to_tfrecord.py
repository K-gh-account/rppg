...
def h5_to_tfrecords(SourceFile, tfrecordsPath, N_train=1e6, N_val=2.5e5, N_test=2.5e5,
    ...
    with h5py.File(SourceFile, 'r') as f:
        ...
        BP = np.round(BP)
        BP = np.transpose(BP)   #行列对换， BP[0]: SBP, BP[1]:DBP
        subject_idx = np.squeeze(np.array(f.get('/subject_idx'))) #去除单维度（此处为最后一维）
    ...
    if divide_by_subject is True:
        ...
        subject_labels, SampSubject_hist = np.unique(subject_idx, return_counts=True) #去除重复subject_idx, 并对每个idx计数
        cumsum_samp = np.cumsum(SampSubject_hist) #依次对subject_idx的样本数量累加
        subject_labels_train = subject_labels[:np.nonzero(cumsum_samp>(N_train+N_val+N_test))[0][0]+1] #全部数据所需的subject_idx列表
        idx_valid = np.nonzero(np.isin(subject_idx,subject_labels_train))[0] #选中的subject_idx所在数据的index
        ...
        idx_val, idx_test = train_test_split(idx_val, test_size=0.5) #只有test_size(float), 按比例分配

    # save ground truth BP values of training, validation and test set in csv-files for future reference
    BP_train = BP[:,idx_train] #切出所需训练数据的BP值
...
if __name__ == "__main__":
    ...
    if len(argv) > 1:
        ...
        HomePath = expanduser("")  #将字符串中的波浪线‘~’转换为主目录或home目录，此处其实没必要
        SourceFile = join(HomePath, 'data', 'MIMICIII_PPG.h5')  #Sourcefile: 待转为TFrecord的H5文件名
        tfrecordsPath = join(HomePath, 'test') #tfrecordsPath: 存放TFrecord的位置
        divbysubj = False
        N_train = 1.5e6
        N_val = 4.5e5
        N_test = 4.5e5
