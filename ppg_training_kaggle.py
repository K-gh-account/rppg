...
def ppg_train_mimic_iii(architecture,
    ...
    for i in range(int(Ntest//batch_size)):
        ...
        test_results = pd.concat([test_results,TestBatchResult])
...
def main():
    ...
    if len(argv) > 1:
        ...
        DataDir = join(HomePath,'test')
        ResultsDir = join(HomePath,'test','results')
        CheckpointDir = join(HomePath,'test','checkpoints')
